...
(function () {
    ...
    // 드롭다운 값은 raw 전송값. none + ST 기본(minimum~maximum) + OpenAI 전용(minimal/xhigh).
    const REASONING_OPTIONS = ['보내지 않기', 'auto', 'none', 'minimum', 'low', 'medium', 'high', 'maximum', 'minimal', 'xhigh'];
    const REASONING_OAI_ONLY = ['minimal', 'xhigh']; // UI에 "(OpenAI 전용)" 라벨
    ...
    // ── 자동매칭 (회사 판별 + reasoning_effort 값 변환) ──────────────────
    // 모델명 문자열로 회사를 추정한다. 프록시 접두/접미(예: [0.03]*/, [次], -thinking)는
    // 부분 문자열 매칭으로 무시한다. Gemini는 프록시 전송 형식이 불확실하여 보류한다.
    ...
    // 자동매칭이 켜진 경우, 회사별로 raw 값을 실제 전송값으로 변환한다.
    // 반환: { value: 변환된값|null(=보내지 않음), label: 표시용 설명 }
    // 입력 가능한 raw 값: minimum, low, medium, high, maximum, minimal, xhigh
    function mapReasoningValue(rawValue, vendor, autoOn) {
        ...
        if (!autoOn || !vendor) {
            return { value: rawValue, label: '' }; // raw 그대로
        }
        // none(추론 끄기)은 어느 회사로도 변환하지 않는다. 변환하면 의미가 정반대가
        // 될 수 있다(예: DeepSeek에서 high로 바뀌면 추론을 끄려다 최대로 켜짐).
        if (rawValue === 'none') {
            return { value: 'none', label: `${vendor} · none(그대로)` };
        }
        if (vendor === 'openai') {
            // OpenAI 정식값: minimal/low/medium/high/xhigh.
            // minimum→minimal, maximum→xhigh (ST의 min→minimal, max→xhigh와 동일)
            if (rawValue === 'minimum') return { value: 'minimal', label: 'OpenAI · minimum→minimal' };
            if (rawValue === 'maximum') return { value: 'xhigh', label: 'OpenAI · maximum→xhigh' };
            return { value: rawValue, label: 'OpenAI' };
        }
        if (vendor === 'deepseek') {
            // ST 매핑: auto→안보냄(위 처리), max(imum)/xhigh→max, 나머지 전부→high
            if (rawValue === 'maximum' || rawValue === 'xhigh') return { value: 'max', label: `DeepSeek · ${rawValue}→max` };
            return { value: 'high', label: `DeepSeek · ${rawValue}→high` };
        }
        if (vendor === 'claude') {
            // Claude 값: low/medium/high/xhigh/max. minimal/minimum 없음 → low로.
            // maximum→max.
            if (rawValue === 'minimum' || rawValue === 'minimal') return { value: 'low', label: `Claude · ${rawValue}→low` };
            if (rawValue === 'maximum') return { value: 'max', label: 'Claude · maximum→max' };
            return { value: rawValue, label: 'Claude' };
        }
    ...
    async function onSettingsReady(generate_data) {
        try {
            ...
            // 커스텀 파라미터 (모델별 저장, raw 그대로 주입. 자동매칭 미적용)
    ...
    // reasoning 전용: minimal/xhigh에 (OpenAI 전용) 라벨
...
<div id="pi_panel">
  ...
  <details class="pi-details">
    <summary class="pi-summary">커스텀 파라미터 (최대 ${MAX_CUSTOM_PARAMS}개)
      ${help('reasoning/verbosity 외의 임의 파라미터를 직접 키·값으로 주입합니다(자동매칭 미적용, 입력한 그대로 전송). 예: 키 thinkingLevel, 값 high. 모델명 기준으로 저장됩니다.')}
    </summary>
  ...
  <div class="pi-section">
        ...
        try {
            ...
            if (isCustom && !cls.reasoningNative) {
                ...
                if (autoOn && vendor) {
                    ...
                    const preview = (rv && !NO_SEND_LABELS.includes(rv))
                        ? mapReasoningValue(rv, vendor, true)
                        : null;
                    const detail = preview && preview.value !== null
                        ? ` · ${esc(rv)}→${esc(preview.value)}` : '';
    ...
    // 커스텀 파라미터 행들을 렌더 (params: [{key,value}], 최대 MAX_CUSTOM_PARAMS)
    function renderCustomParams(params) {
        ...
        // 빈 행 하나는 항상 보이게(추가 입력용), 단 최대치 미만일 때만
        ...
        const addBtn = list.length < MAX_CUSTOM_PARAMS
            ? `<div class="pi-row"><button id="pi_cp_add" class="menu_button">+ 파라미터 추가</button></div>` : '';
    ...
    function bindEvents() {
        ...
        if (automap) {
            metaGet('automap_on', true).then((on) => { automap.checked = !!on; });
        ...
        if (save) save.addEventListener('click', async () => {
            try {
                ...
                const customParams = collectCustomParams()
                    .map((cp) => ({ key: String(cp.key || '').trim(), value: String(cp.value || '').trim() }))
                    .filter((cp) => cp.key && cp.value);
                await dbPut(STORE_MODELS, { model, reasoning, verbosity, customParams, updatedAt: Date.now() });

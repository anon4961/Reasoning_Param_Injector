/*
 * Reasoning Param Injector v1.6
 * ------------------------------------------------------------
 * 목적: Custom(OpenAI-compatible) 엔드포인트에서, 현재 활성 모델 기준으로
 *       reasoning_effort / verbosity 를 모델명 단위로 저장해두고,
 *       요청 직전에 "화이트리스트 자동 판독"을 거쳐 주입한다.
 *
 * 핵심 동작 (사용자 확정 설계):
 *   - 요청 직전, 활성 모델명을 ST 화이트리스트와 자동 대조한다.
 *   - 화이트리스트 통과 모델(=ST가 자체 전송) → 주입하지 않고 "불필요" 안내.
 *     (덮어쓰기/충돌을 일으키지 않는다.)
 *   - 미통과 모델(=ST 미전송) → 우리 값을 주입한다.
 *   - reasoning_effort 와 verbosity 는 각각 따로 판독한다.
 *     (verbosity 는 /^gpt-5/, reasoning 은 정확 일치)
 *
 * 안전 원칙:
 *   - ST 본체 설정(oai_settings / settings.json)을 수정하지 않는다.
 *   - generate_data 에 해당 키가 이미 있으면 절대 덮어쓰지 않는다.
 *   - 데이터는 전용 IndexedDB(st_param_injector)에만 저장.
 *   - 모든 핸들러는 try-catch 격리. 오류가 나도 본체 생성 흐름을 막지 않는다.
 *
 * 한계(정직):
 *   - 화이트리스트 판독은 "ST가 전송하는지"만 안다. 프록시가 실제 모델에
 *     전달하는지는 별개 → 사용자가 직접 실험으로 확인해야 한다.
 *   - 확장은 응답/Termux 로그를 읽지 못한다(fetch 후킹 없음).
 *     로그는 "주입했다"까지만 보장하며, 실제 성공은 Termux 확인 몫.
 */

...
### Custom parameters (up to 5)

Inject arbitrary key/value parameters beyond reasoning/verbosity (collapsible
menu). Sent **exactly as entered** with no auto-mapping, stored per model name.
Example: if your Gemini proxy accepts `thinkingLevel`, send key `thinkingLevel`,
value `high`.
